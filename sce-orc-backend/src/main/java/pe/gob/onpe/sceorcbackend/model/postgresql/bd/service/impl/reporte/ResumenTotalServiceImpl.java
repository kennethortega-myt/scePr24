...
public class ResumenTotalServiceImpl implements ResumenTotalService {
	...
	public ResumenTotalServiceImpl(ActaRepository actasRepository, UtilSceService utilSceService,
	...
	private Map<String, Object> getParametrosPorcentaje(Map<String, Object> parametros, List<DetalleResumenTotal> detalle) {
		DetalleResumenTotal totalPorcentajes =  getTotalesResumenPorcentajePromedio(getTotalesResumenCifras(detalle), detalle.size());
	...
	private List<DetalleResumenTotal> resumenTotalPorcentajes(FiltroResumenTotalDto filtro) {
		try {
			...
			if(detalleTotal != null && !detalleTotal.isEmpty()) {
				...
				DetalleResumenTotal totalesPorcentaje = getTotalesResumenPorcentajePromedio(getTotalesResumenCifras(detalleTotal), detalleTotal.size());
            	...
            	this.logService.registrarLog(filtro.getUsuario(), Thread.currentThread().getStackTrace()[1].getMethodName(),
            			this.getClass().getSimpleName(), "Se consultó el Reporte de Resumen Total porcentajes.", filtro.getCentroComputo(), ConstantesComunes.LOG_TRANSACCIONES_AUTORIZACION_NO, ConstantesComunes.LOG_TRANSACCIONES_ACCION);
	...
	private List<DetalleResumenTotal> resumenTotalCifras(FiltroResumenTotalDto filtro) {
		try {
			...
			if(detalleTotal != null && !detalleTotal.isEmpty()) {
				...
				try {
	            	this.logService.registrarLog(filtro.getUsuario(), Thread.currentThread().getStackTrace()[1].getMethodName(),
	            			this.getClass().getSimpleName(), "Se consultó el Reporte de Resumen Total cifras.", filtro.getCentroComputo(), ConstantesComunes.LOG_TRANSACCIONES_AUTORIZACION_NO, ConstantesComunes.LOG_TRANSACCIONES_ACCION);
	            } catch (Exception e) {
	            	logger.error("Error al registra log de Reporte de Resumen Total cifras: ", e);
	            }
	...
	private DetalleResumenTotal getTotalesResumenCifras(List<DetalleResumenTotal> detalle) {
		return DetalleResumenTotal
				...
				.digActasPorcentaje(detalle.parallelStream().reduce(0d, (suma, resumen) -> suma + resumen.getDigActasPorcentaje(), Double::sum))
				.digListaElectoresPorcentaje(detalle.parallelStream().reduce(0d, (suma, resumen) -> suma + resumen.getDigListaElectoresPorcentaje(), Double::sum))
				.digHojasAsistenciaPorcentaje(detalle.parallelStream().reduce(0d, (suma, resumen) -> suma + resumen.getDigHojasAsistenciaPorcentaje(), Double::sum))
				.digResolucionesPorcentaje(detalle.parallelStream().reduce(0d, (suma, resumen) -> suma + resumen.getDigResolucionesPorcentaje(), Double::sum))

				.txActasPorcentaje(detalle.parallelStream().reduce(0d, (suma, resumen) -> suma + resumen.getTxActasPorcentaje(), Double::sum))
				.txResolucionPorcentaje(detalle.parallelStream().reduce(0d, (suma, resumen) -> suma + resumen.getTxResolucionPorcentaje(), Double::sum))
				
				.omisosMmPorcentaje(detalle.parallelStream().reduce(0d, (suma, resumen) -> suma + resumen.getOmisosMmPorcentaje(), Double::sum))
				.omisosVotantesPorcentaje(detalle.parallelStream().reduce(0d, (suma, resumen) -> suma + resumen.getOmisosVotantesPorcentaje(), Double::sum))
				.omisosMmActaEscrutinioPorcentaje(detalle.parallelStream().reduce(0d, (suma, resumen) -> suma + resumen.getOmisosMmActaEscrutinioPorcentaje(), Double::sum))
				.omisosPersonerosPorcentaje(detalle.parallelStream().reduce(0d, (suma, resumen) -> suma + resumen.getOmisosPersonerosPorcentaje(), Double::sum))
	...
	private List<DetalleResumenTotal> getDetalleResumenCifras(FiltroResumenTotalDto filtro) {
		try {
			...
			if(resumenMap == null || resumenMap.size() == 0) return new ArrayList<>();
	...
	private DetalleResumenTotal getTotalesResumenPorcentajePromedio(DetalleResumenTotal totales, int filas) {
		Double totalMesasAinstalar = totales.getMesasAInstalar();
		Double div1 = totales.getMesasAInstalar() - ( totales.getMesasNoInstal() + totales.getActasSiniestradas() + totales.getActasExtraviadas() );
		...
		return DetalleResumenTotal
				...
				.digActas( totales.getDigActasPorcentaje() / filas)
				.digListaElectores( totales.getDigListaElectoresPorcentaje() / filas )
				.digHojasAsistencia( totales.getDigHojasAsistenciaPorcentaje() / filas )
				.digResoluciones( totales.getDigResolucionesPorcentaje() / filas )
				
				.txActas( totales.getTxActasPorcentaje() / filas )
				.txResolucion( totales.getTxResolucionPorcentaje() / filas )
				
				.omisosMm( totales.getOmisosMmPorcentaje() / filas )
				.omisosVotantes( totales.getOmisosVotantesPorcentaje() / filas )
				.omisosMmActaEscrutinio( totales.getOmisosMmActaEscrutinioPorcentaje() / filas )
				.omisosPersoneros( totales.getOmisosPersonerosPorcentaje() / filas)
